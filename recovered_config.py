...
class RunConfig:
    ...
    output_path: Path = Path('~/Attend-and-Excite/outputs')
...
@dataclass
class BatchConfig:
    # File with prompts and token indices
    prompts: str = 'prompts.json'
    # Which random seeds to use when generating
    # seeds: List[int] = field(default_factory=lambda: [42])
    seeds: List[int] = field(default_factory=lambda: [random.randint(0,9999999) for i in range(10)])
    # Path to save all outputs to
